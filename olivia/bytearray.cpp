...
bool ByteArray::isLittleEndian() const {
    return m_endian == OLIVIA_LITTLE_ENDIAN;
}

void ByteArray::setIsLittleEndian(bool val) {
    if(val) {
        m_endian = OLIVIA_LITTLE_ENDIAN;
    } else {
        m_endian = OLIVIA_BIG_ENDIAN;
    }
}
...
// 	// uint32_t 的极限是5个字节   32 / 7 
//     uint8_t tmp[5];
//     uint8_t i = 0;
// 	// google压缩算法，每个字节的第一位作为标志位，为1时，表示还有数据
//     while(value >= 0x80) {
...
// 	// uint64_t的极限是10个字节  64 /7 每7位设置一位标志位
//     uint8_t tmp[10];
//     uint8_t i = 0;
//     while(value >= 0x80) {
//         tmp[i++] = (value & 0x7F) | 0x80;
