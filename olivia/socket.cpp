...
Socket::ptr Socket::CreateTCP(olivia::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(olivia::Address::ptr address) {
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if(!isValid()) {
        newSock();
        if(OLIVIA_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if(OLIVIA_UNLIKELY(addr->getFamily() != m_family)) {
...
/*
    ...
    if(uaddr) {
        ...
        if(sock->connect(uaddr)) {
            ...
            olivia::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }
*/
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, olivia::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, olivia::IOManager::WRITE);
}
...
SSLSocket::ptr SSLSocket::CreateTCP(olivia::Address::ptr address) {
